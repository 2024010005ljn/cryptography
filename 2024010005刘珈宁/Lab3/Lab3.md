...
**答案：** <u>6c73d5240a948c86981bc297804a</u>
...
**答案：**
***
1. 当 $\mathrm{Adv}_{\mathrm{PRG}}[A, G]$≈1时：算法 A能高效区分伪随机串与真随机串，G伪随机性差、安全性弱。
2. 当 $\mathrm{Adv}_{\mathrm{PRG}}[A, G]$≈0时：算法 A无法区分伪随机串与真随机串，G伪随机性好、安全性强。
***
...
**答案：**
***
事件 $A(G(k)) = 1$ 等价于 $\operatorname{MSB}(G(k)) = 1$，因为：对于 $K$ 中 $\frac{2}{3}$ 的密钥 $k$，有 $\operatorname{MSB}(G(k)) = 1$，因此：
$$
\Pr\left[A(G(k)) = 1\right] = \Pr\left[\operatorname{MSB}(G(k)) = 1\right] = \frac{2}{3}
$$
***
...
**答案：**
***
$r$ 是从 $\{0,1\}^n$ 中均匀随机选取的真随机串，其最高有效位 $\operatorname{MSB}(r)$ 只有 $0$ 和 $1$ 两种等概率取值，因此：
$$
\Pr\left[\operatorname{MSB}(r) = 1\right] = \frac{1}{2}
$$

同理，事件 $A(r) = 1$ 等价于 $\operatorname{MSB}(r) = 1$，因此：
$$
\Pr\left[A(r) = 1\right] = \Pr\left[\operatorname{MSB}(r) = 1\right] = \frac{1}{2}
$$
***
...
根据PRG优势的标准定义：
$$
\operatorname{Adv}_{\text{PRG}}[A, G] = \left| \Pr\left[A(G(k)) = 1\right] - \Pr\left[A(r) = 1\right] \right|
$$

将(a)、(b)的计算结果代入公式：
$$
\operatorname{Adv}_{\text{PRG}}[A, G] = \left| \frac{2}{3} - \frac{1}{2} \right|
$$
...
取绝对值后得到最终结果：
$$
\operatorname{Adv}_{\text{PRG}}[A, G] = \frac{1}{6}
$$
***
**(d)** 该 PRG 是否安全？为什么？
...
***
根据伪随机生成器（PRG）的安全定义：
一个 PRG 是安全的，当且仅当**对于任意多项式时间的区分器 \(A\)**，其优势
$$
\operatorname{Adv}_{\text{PRG}}[A, G] = \left| \Pr_{k \leftarrow K}[A(G(k))=1] - \Pr_{x \leftarrow \{0,1\}^n}[A(x)=1] \right|
$$
是**可忽略的**（即随着安全参数增大，优势趋近于 0）。

在本题中，我们构造的区分器 \(A\) 满足 $\operatorname{Adv}_{\text{PRG}}[A, G] = \frac{1}{6}$，该优势是一个**常数**，不随安全参数变化而减小，不属于可忽略的范畴。
...
**答案：**
***
完美保密性的定义为：对于任意明文 \( m \in \mathcal{M} \) 和密文 \( c \in \mathcal{C} \)，有

$$
\Pr_k\left[ E(k, m) = c \right] = \Pr[c]
$$
即**加密得到密文 \( c \) 的概率与明文 \( m \) 无关**，仅由密文本身的分布决定。
***
...
c = E(k, m) = m \oplus k
$$
其中 \( \oplus \) 表示按位异或运算。对等式变形可得：
$$
k = m \oplus c
$$

对于任意固定的明文 \( m \in \mathcal{M} \) 和密文 \( c \in \mathcal{C} \)，**恰好存在 1 个密钥 \( k \)** 使得 \( E(k, m) = c \)。
***
...
**答案：**
***
设密钥空间为 \( \mathcal{K} \)，密钥 \( k \) 从 \( \mathcal{K} \) 中均匀随机选取，因此每个密钥被选中的概率为 \( \frac{1}{|\mathcal{K}|} \)。

由(b)的结论可知：对任意 \( m \in \mathcal{M} \) 和 \( c \in \mathcal{C} \)，满足 \( E(k, m) = c \) 的密钥 \( k \) 恰好有 1 个，因此：
$$
\Pr_k\left[ E(k, m) = c \right] = \frac{1}{|\mathcal{K}|}
$$

在OTP中，密文空间 \( \mathcal{C} \) 与密钥空间 \( \mathcal{K} \) 大小相等，且密文服从均匀分布，因此任意密文 \( c \) 的先验概率为：
$$
\Pr[c] = \frac{1}{|\mathcal{K}|}
$$

因此对任意 \( m, c \)，有：
$$
\Pr_k\left[ E(k, m) = c \right] = \frac{1}{|\mathcal{K}|} = \Pr[c]
$$

完全满足(a)中完美保密性的定义，故**一次性密码本（OTP）具有完美保密性**。
***
---
...
**答案：**
***
语义安全的核心定义：一个加密方案是语义安全的，当且仅当**不存在多项式时间的高效算法**，能从密文中推断出明文的任何非平凡有效信息。

已知存在高效算法 $A$，它总能从密文 $c$ 中推断出明文 $m$ 的最低有效位 $\operatorname{LSB}(m)$。这说明攻击者可以通过密文获取明文的**确定比特信息**，直接违反了语义安全的定义，因此加密方案 $\mathbb{E}$ 不是语义安全的。
***
...
***
1. 选择挑战明文
攻击者 $B$ 选择两条满足 $\operatorname{LSB}$ 不同的明文：
$$
m_0: \text{满足 } \operatorname{LSB}(m_0) = 0, \quad m_1: \text{满足 } \operatorname{LSB}(m_1) = 1
$$
将 $m_0, m_1$ 发送给挑战者。

2. 利用算法 $A$ 猜测挑战比特 $b$
挑战者返回密文 $c = E(k, m_b)$（其中 $b \in \{0,1\}$ 为随机挑战比特）。
$B$ 调用子程序 $A$ 对密文 $c$ 进行计算，得到 $\operatorname{LSB}(m_b)$：
- 若 $\operatorname{LSB}(m_b) = 0$，则 $B$ 猜测 $b' = 0$；
- 若 $\operatorname{LSB}(m_b) = 1$，则 $B$ 猜测 $b' = 1$。
***
...
语义安全优势的定义为：
$$
\operatorname{Adv}_{\text{SS}}[B, \mathbb{E}] = \left| \Pr[b' = b] - \frac{1}{2} \right|
$$

由题设，算法 $A$ 总能正确推断出 $\operatorname{LSB}(m_b)$，因此攻击者 $B$ 的猜测 $b'$ 与真实挑战比特 $b$ 完全一致，即：
$$
\Pr[b' = b] = 1
$$
...
代入优势公式计算：
$$
\operatorname{Adv}_{\text{SS}}[B, \mathbb{E}] = \left| 1 - \frac{1}{2} \right| = \frac{1}{2}
$$
***
...
**答案：**
***
OTP的密钥 $k$ 是从 $\{0,1\}^n$ 中**均匀随机选取**的，即对任意 $k \in \{0,1\}^n$，$\Pr[k] = \frac{1}{2^n}$。

对于任意固定的 $m_0 \in \{0,1\}^n$ 和任意目标密文 $c^* \in \{0,1\}^n$，令 $k^* = c^* \oplus m_0$。
则：
$$
...
**答案：**
***
密钥 $k$ 是均匀随机的，对于任意固定的 $m_1 \in \{0,1\}^n$ 和任意目标密文 $c^* \in \{0,1\}^n$，令 $k^* = c^* \oplus m_1$。
则：
$$
\Pr[c = c^*] = \Pr[k \oplus m_1 = c^*] = \Pr[k = k^*] = \frac{1}{2^n}
$$
因此，$c$ 在 $\{0,1\}^n$ 上也是均匀分布的。
***
...
***
由(a)和(b)可知：
- $\text{EXP}(0)$ 中密文 $c$ 的分布是 $\{0,1\}^n$ 上的均匀分布；
- $\text{EXP}(1)$ 中密文 $c$ 的分布也是 $\{0,1\}^n$ 上的均匀分布。
...
$$
其中 $p = \Pr[A(k \oplus m_0) = 1] = \Pr[A(k \oplus m_1) = 1]$。
***
